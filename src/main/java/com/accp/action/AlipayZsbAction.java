...
import com.accp.action.AlipayConfig;
...
public class AlipayZsbAction {// response
	...
	@RequestMapping("viewOrder")
	public void viewOrder(HttpServletRequest req, Model mod, HttpServletResponse rep,HttpSession session,Float recordinandout) throws AlipayApiException, IOException {
		//获得初始化的AlipayClient
...
//		//设置请求参数
		...
		alipayRequest.setReturnUrl(AlipayConfig.return_url);
		alipayRequest.setNotifyUrl(AlipayConfig.notify_url);
		Integer recordid = biz.findGoldNoteMaxRecordId();
		//商户订单号，商户网站订单系统中唯一订单号，必填
		String out_trade_no =recordid.toString();
		//付款金额，必填
		String total_amount =goldnotes.getRecordinandout().toString();//recordInAndOut
		//订单名称，必填
	    String subject =lo.getUserid().toString()+"账号充值";
		//商品描述，可空
		String body ="";
		...
		Integer usermoney =(int) (lo.getUser().getUsermoney()+recordinandout);
		biz.updateUserMoney(usermoney, lo.getUserid());
		lo.getUser().setUsermoney(usermoney);
		String result = alipayClient.pageExecute(alipayRequest).getBody();
		rep.setContentType("text/html;charset=" + AlipayConfig.charset);
		rep.getWriter().write(result);// 直接将完整的表单html输出到页面
		rep.getWriter().flush();
